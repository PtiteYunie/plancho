...
class User
{
    ...
    public function userExists()
    {
        ...
        if ($userExists->execute([$this->email]) > 0) {
    ...
    public function connectUser()
    {
        if ($this->userExists() !== false) { // L'utilisateur exite : On récupère toutes les informations le concernant
            ...
            if (hash_equals(sha1($this->password),
                $userExists['password'])) { // On compare le mot de passe rentré dans le formulaire de connexion et celui de la BDD
                ...
                $query->execute([$userExists['id']]);


                $user = new User($userExists['id'], $userExists['username'], $userExists['firstName'],
                    $userExists['lastName'],
